  ...
  return (
    <Card className="p-4">
      <div className="flex items-start justify-between">
        ...
        <Icon size={16} style={{ color: STAT_COLOR[tone] }} />
      </div>
      <div className="mt-2 text-[2rem] font-extrabold leading-none tracking-tight text-[var(--ink)]">
        <CountUp value={value} decimals={decimals} suffix={suffix} />
      </div>
      <div className="mt-1.5 flex items-end justify-between gap-2">
        <span className="text-[0.72rem] text-[var(--ink-muted)]">{detail}</span>
        {series && <Sparkline data={series} width={80} height={26} stroke={STAT_COLOR[tone]} />}
      </div>
    </Card>
  );
}
