...
// 7.5h per non-available weekday (Training / Leave / Sick / Booked, or a public holiday).
//
// Every capacity and availability helper below tests `!== "Available"` rather than naming
// statuses, which is why adding Booked needed no logic change here: a new hard status costs
...
// label it. `pattern` is an optional background-image layered over `bg`; only Booked uses it.
export const CALENDAR_STATUS_META = {
  ...
  Booked: {
    ...
    pattern: "repeating-linear-gradient(45deg, var(--cal-booked-stripe) 0 4px, transparent 4px 8px)",
  },
  "Public Holiday": { label: "Public Holiday", ink: "var(--cal-holiday)", bg: "var(--cal-holiday-bg)" },
};

// Look up a status's presentation, tolerating one this build has never heard of. Callers used
// to index CALENDAR_STATUS_META directly and read `.bg` off the result, so a status the client
// didn't know threw and took the whole availability view down with it. That is a live hazard
// whenever a status is renamed: rows written by the previous build outlive it, and during the
// "Blocked" → "Booked" rename 23 real rows sat in exactly that gap. Render it, don't die.
export function metaFor(status) {
  return CALENDAR_STATUS_META[status] || {
    label: String(status || "Unknown"),
    ink: "var(--ink-muted)",
    bg: "var(--surface-sunken)",
    reason: `On ${String(status || "unknown").toLowerCase()}`,
  };
}
...
export function unavailableReason(staffId, startISO, endISO, entriesByKey, holidaySet) {
  for (const date of datesInRange(startISO, endISO)) {
    ...
    const reason = metaFor(status).reason;
